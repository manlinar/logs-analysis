...
# table1 is a table that consists of 2 columns.The 1st is the column of the
# days and the 2nd is the column
# of the number of all the requests that happened each day.

# table2 is a table that consinsts of the column of days and the column with
# the number of requests with status '404 NOT FOUND' that happened each day.
...
query3 = """
                ...
                , table3 AS (
                SELECT table1.day,
                table2.number_of_errors::decimal * 100 /
                table1.number_of_requests
                AS error_percentage
                FROM table1
...
for result in results:
    print '{day: %B %e, %Y}  -  {table3:.3}% errors '.format(
        day=result[0], table3=result[1])
print ''
